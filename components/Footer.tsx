      ...
      <div className="mx-auto max-w-6xl px-5 py-16">
        ...
        <div className="mt-12 grid gap-10 sm:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-3 space-y-1.5 text-sm">
              {[['/acervo','Acervo de imagens'],['/documentario','Documentário Música & Drama'],['/linha-do-tempo','Linha do tempo'],['/memorias','Memórias e curiosidades'],['/livro-de-memorias','Livro de memórias'],['/visita-guiada','Visita guiada'],['/fontes','Pesquisa e fontes']].map(([h,l]) => (
                <li key={h}><Link href={h} className="text-cream/80 hover:text-gold">{l}</Link></li>
              ))}
            </ul>
          </div>
        ...
        <div className="mt-12 border-t border-cream/15 pt-6 text-[0.7rem] leading-relaxed text-cream/65">
          <p className="mt-2">© <AnoAtual /> Theatro Municipal de São João da Boa Vista. História, arte e memória.</p>
        </div>
      </div>
    </footer>
  );
