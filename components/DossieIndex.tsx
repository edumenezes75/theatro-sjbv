...
export default function DossieIndex() {
  ...
  return (
    <nav className="sticky top-28 max-h-[80vh] overflow-y-auto pr-1" aria-label="Capítulos do episódio">
      <p className="mb-4 font-sans text-xs uppercase tracking-eyebrow text-curtain/70 dark:text-gold/70">Exposição</p>
          ...
          return (
            <li key={it.id}>
              <button onClick={() => go(it.id)} aria-current={on ? 'true' : undefined} className="group flex items-start gap-3 text-left">
                <span className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border font-sans text-xs tabular-nums transition-colors ${on ? 'border-curtain bg-curtain text-cream dark:border-gold dark:bg-gold dark:text-night' : 'border-ink/25 text-ink/55 group-hover:border-curtain/60 dark:border-cream/25 dark:text-cream/65'}`}>{i + 1}</span>
                <span className={`pt-0.5 font-sans text-sm leading-snug transition-colors ${on ? 'text-curtain dark:text-gold' : 'text-ink/55 group-hover:text-ink/80 dark:text-cream/65 dark:group-hover:text-cream/80'}`}>{it.text}</span>
              </button>
            </li>
          );
        })}
