  ...
  return (
    <figure>
      ...
      >
        ...
        <span className="pointer-events-none absolute left-3 top-3 rounded-full bg-ink/70 px-2.5 py-1 font-sans text-xs uppercase tracking-eyebrow text-gold">{par.labelAntes ?? 'Em obra'}</span>
        <span className="pointer-events-none absolute right-3 top-3 rounded-full bg-ink/70 px-2.5 py-1 font-sans text-xs uppercase tracking-eyebrow text-cream">{par.labelDepois ?? 'Restaurado'}</span>
...
function Lightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-ink/95 p-3 sm:p-6" onClick={onClose} role="dialog" aria-modal="true" aria-label={alt}>
      ...
      <p className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-ink/60 px-3 py-1 font-sans text-xs uppercase tracking-eyebrow text-cream/85">Toque na imagem para ampliar · Esc fecha</p>
    </div>
  );
}
          ...
          <li key={i} className="relative">
            ...
            >
              ...
              <span className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink/85 via-ink/20 to-transparent p-2.5 pt-7">
                <span className="font-sans text-xs uppercase tracking-eyebrow text-gold">{im.label}</span>
              </span>
