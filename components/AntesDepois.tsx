...
type Par = { id: string; title: string; caption: string; antes: string; depois: string; w: number; h: number; credit: string; mode?: string; labelAntes?: string; labelDepois?: string };
...
function Tile({ src, alt, label }: { src: string; alt: string; label: string }) {
  return (
...
function LadoALado({ par }: { par: Par }) {
  return (
    <figure>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Tile src={par.antes} alt={`${par.title} — ${par.labelAntes ?? 'antes'}`} label={par.labelAntes ?? 'Antes'} />
        <Tile src={par.depois} alt={`${par.title} — ${par.labelDepois ?? 'depois'}`} label={par.labelDepois ?? 'Depois'} />
      </div>
      <figcaption className="mt-3 max-w-reading font-sans text-sm text-ink/70 dark:text-cream/70">
        <span className="font-display text-base font-medium text-ink dark:text-cream">{par.title}.</span> {par.caption}
        <span className="mt-0.5 block text-xs italic text-ink/75 dark:text-cream/75">{par.credit}</span>
...
export default function AntesDepois({ pares }: { pares: Par[] }) {
  return <div className="grid gap-10 md:grid-cols-2">{pares.map((p) => p.mode === 'lado-a-lado' ? <LadoALado key={p.id} par={p} /> : <Slider key={p.id} par={p} />)}</div>;
}
